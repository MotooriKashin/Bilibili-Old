...
namespace API {
    class Sign {
        ...
        static sign(url: string, obj: Record<string, string | number> = {}, id: number | string = 0) {
            ...
            table.sign = id === 3 && table.api ? (md5(Format.objUrl("", { api: decodeURIComponent(<string>table.api) }) + this.keySecret[1])) : (md5(Format.objUrl("", table) + this.keySecret[1]));
