...
class Xhr {
     ...
     */
    false(url, responseType = "text", credentials = true) {
    ...
    GM(url, headers = {}) {
        return new Promise((resolve, reject) => {
            BLOD.xmlhttpRequest({
                ...
                headers: headers,
                onload: (xhr) => {
                    BLODxhrLog.push([BLOD.timeFormat(), url, (String(xhr.responseText).startsWith("{") ? JSON.parse(xhr.responseText) : xhr.responseText)]);
...
BLOD.objUrl = (url, obj) => { return new format.objUrl(url, obj) }
BLOD.urlObj = (url) => { return new format.urlObj(url) }
