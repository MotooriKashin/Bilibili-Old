...
(function () {
    ...
    API.switchVideo(async () => {
        try {
            ...
            let data = await API.xhr({ url: API.objUrl("https://api.bilibili.com/x/web-interface/archive/stat", { "aid": String(API.aid) }) }); // 获取分集数据
        ...
        } catch (e) { API.debug.trace(e, "episodeData.js") }
    })
})();