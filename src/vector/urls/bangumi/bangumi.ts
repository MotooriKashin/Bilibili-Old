...
(function () {
    ...
    try {
        ...
        let id = API.path[5].startsWith('ep') ? (<string[]>location.href.match(/[0-9]+/))[0] : null;
        id = id || (data[0] && data[0].split(":")[1]) || null;
        try {
            // 准备__INITIAL_STATE__
            if (API.path[5].startsWith('ss')) {
                data = API.xhr({
                    url: API.objUrl("https://bangumi.bilibili.com/view/web_api/season", { season_id: (<string[]>location.href.match(/[0-9]+/))[0] }),
                    async: false
                });
            } else if (API.path[5].startsWith('ep')) {
                data = API.xhr({
                    url: API.objUrl("https://bangumi.bilibili.com/view/web_api/season", { ep_id: (<string[]>location.href.match(/[0-9]+/))[0] }),
                    async: false
                });
            }
            API.importModule("bangumi-season.js", { __INITIAL_STATE__: data, epid: id });
        } catch (e) {
            if (!config.videoLimit) throw e;
            // 尝试访问泰区服务器
            if (API.path[5].startsWith('ss')) {
                data = API.xhr({
                    url: API.objUrl(`${config.limitServer || "https://api.global.bilibili.com"}/intl/gateway/v2/ogv/view/app/season`, { season_id: (<string[]>location.href.match(/[0-9]+/))[0] }),
                    async: false
                });
            } else if (API.path[5].startsWith('ep')) {
                data = API.xhr({
                    url: API.objUrl(`${config.limitServer || "https://api.global.bilibili.com"}/intl/gateway/v2/ogv/view/app/season`, { ep_id: (<string[]>location.href.match(/[0-9]+/))[0] }),
                    async: false
                });
            }
        ...
        if (isBANGUMI__INITIAL_STATE__(API.__INITIAL_STATE__)) {
            if (API.__INITIAL_STATE__?.epInfo?.badge === "互动") return API.toast.warning("这似乎是个互动番剧！", "什么！番剧也能互动？", "可惜旧版播放器不支持 ಥ_ಥ");
    ...
    } catch (e) { API.debug.trace(e, "bangumi.js", true) }
})();
