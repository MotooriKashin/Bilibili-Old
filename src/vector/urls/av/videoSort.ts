        ...
        if (!API.tid) {
            try {
                API.tid = API.jsonCheck(await API.xhr({
                    url: API.objUrl("https://api.bilibili.com/x/web-interface/view", { aid: <string>API.aid })
                })).data.tid;
                if (!(API.tid in sort)) return;
                ...
                (<HTMLAnchorElement>nodes[2].childNodes[0]).innerText = sort[sort[API.tid][0]][1];
                (<HTMLAnchorElement>nodes[4].childNodes[0]).href = sort[API.tid][2];
                (<HTMLAnchorElement>nodes[4].childNodes[0]).innerText = sort[API.tid][1];
            } catch (e) { API.debug.trace(e, "videoSort.js") }
        }
            ...
            (<HTMLAnchorElement>nodes[1].childNodes[0]).innerText = sort[sort[API.tid][0]][1];
            (<HTMLAnchorElement>nodes[2].childNodes[0]).href = sort[API.tid][2];
            (<HTMLAnchorElement>nodes[2].childNodes[0]).innerText = sort[API.tid][1];
        }
    })
})();