        ...
        try {
            if (aid) {
                ...
                try {
                    // 尝试访问bangumi接口
                    let data;
                    if (ssid) data = await API.xhr({ url: API.objUrl("https://bangumi.bilibili.com/view/web_api/season", { season_id: ssid }) });
                    else if (epid) data = await API.xhr({ url: API.objUrl("https://bangumi.bilibili.com/view/web_api/season", { ep_id: epid }) });
                    if (data) {
                    ...
                    e = Array.isArray(e) ? e : [e];
                    let data;
                    if (epid) API.debug.error("获取视频信息出错：epid：" + epid, "HOST：https://bangumi.bilibili.com/view/web_api/season", ...e);
                    else if (ssid) API.debug.error("获取视频信息出错：ssid：" + ssid, "HOST：https://bangumi.bilibili.com/view/web_api/season", ...e);
                    try {
                        if (epid) {
