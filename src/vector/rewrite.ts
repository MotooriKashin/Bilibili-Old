...
(function () {
    ...
    if (API.path[2] == "message.bilibili.com") API.addCss(GM.getResourceText("message.css"));
    if (window.self == window.top && API.path[2] == 'www.bilibili.com') document.domain = "bilibili.com";
    if (location.href.includes("message.bilibili.com/pages/nav/index_new_sync")) API.addCss(GM.getResourceText("imroot.css"));
