...
(function () {
    class AllDanmaku {
        ...
        constructor(callback?: Function) {
            ...
            this.pubdate = API.timeFormat(<any>this.pubdate, true).split(" ")[0]; // 视频上传日期
         ...
         */
        async init() {
            if (!API.uid) return API.toast.warning("本功能需要登录！")
            if (!this.pubdate) return API.toast.warning("投稿日期获取失败！无法获取全部弹幕！");
            try {
                // 获取当日日期
                ...
                API.toast("数据返回！已获取弹幕数：" + API.unitFormat(this.danmaku.length));
                this.arrT = this.time.split("-");
                // 如果当天不是投稿日，转入日期检查
            ...
            try {
                ...
                if (data && data[0]) {
                    ...
                    if (this.timeT) {
                        ...
                        API.toast(`技能冷却中。。。请稍待 ${config.allDanmakuDelay} 秒钟`);
                        ...
                        API.toast(`获取前一个月数据 ${this.arrT.slice(0, 2).join("-")} 请稍待 ${config.allDanmakuDelay} 秒钟`);
