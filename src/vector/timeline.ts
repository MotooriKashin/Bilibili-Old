...
namespace API {
    ...
    doWhile(() => document.querySelector<any>("#bili_bangumi > .bangumi-module")?.__vue__ || (<any>window)?.__INITIAL_STATE__, async d => {
        try {
            const index = await url.getJson("app.bilibili.com/x/v2/activity/index", { page_id: 167998 });
            const item: {
                item_id: number,
                title: string;
            }[] = index.data.cards[0].item[0].item;
            await Promise.all(item.reduce((s, d) => {
                s.push(url.getJson("app.bilibili.com/x/v2/activity/inline", { page_id: d.item_id }).then(t => {
            ...
            toast.error("获取港澳台番剧时间线出错 ಥ_ಥ");
            debug.error("港澳台番剧时间线", e);
        }
    });
}