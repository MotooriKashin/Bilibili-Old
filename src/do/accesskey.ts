...
namespace API {
    class Accesskey {
        ...
        constructor() {
            ...
            addElement("div", { style: 'text-align: center;font-size: 16px;font-weight: bold;margin-bottom: 10px;' }, this.box, `<span>账户授权<span>`);
            addElement("div", { style: 'margin-bottom: 10px;' }, this.box, `<div>授权代理服务器使用您的账户权限，以在限制视频等操作中继承您的大会员权益。
            ...
            const body = addElement("div", { style: "display: flex;align-items: center;justify-content: space-around;margin-bottom: 10px;" }, this.box);
            this.enable = body.appendChild(ElementComponent.button(() => { this.access() }, "开始授权", 3));
            this.disable = body.appendChild(ElementComponent.button(() => { this.abort() }, "取消授权", 10));
            this.box.appendChild(ElementComponent.hr());
            this.foot = addElement("div", { style: "display: flex;align-items: center;justify-content: space-around;" }, this.box);
            this.flesh();
        }
        /** 重新获取鉴权按钮 */
        flesh() {
            if (this.access_key) {
                const temp = ElementComponent.button(() => { this.access() }, "重新授权", 3);
                ...
                const temp = ElementComponent.button(() => { this.access() }, "开始授权", 3);
                this.enable.replaceWith(temp);
                this.enable = temp;
        ...
        /** 请求移动端鉴权 */
        async access() {
            if (!uid) return (toast.warning("请先登录！"), biliQuickLogin()); // 登录状态
