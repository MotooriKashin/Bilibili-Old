import { bezier } from "../../runtime/lib/cubicBezier";
...
import { subArray } from "../../runtime/format/subArray";
...
import bannerStyle from "./animated-banner.html";
    ...
    jsonphookasync("api.bilibili.com/x/web-show/res/loc", undefined, async url => {
        ...
        if (!loc || !header) {
            rqs = await Promise.all([
                fetch(objUrl(url, obj)).then(d => d.json()),
