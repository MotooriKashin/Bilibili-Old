...
async function getLostVideo(aid: number) {
    let result = []; // 失效视频信息缓存
    try { // 尝试访问Biliplus
        let data: any = await xhr.GM(`https://www.biliplus.com/video/av${aid}`);
    ...
    if (!result[0] || !result[1]) {
        try { // 标题或封面无效，尝试访问Biliplus CID缓存库
            let data: any = await xhr.GM(`https://www.biliplus.com/all/video/av${aid}/`);
    ...
    if (!result[0] || !result[1]) {
        try { // 标题或封面依旧无效，尝试访问jijidown
            let data: any = await xhr.GM(`https://www.jijidown.com/video/${aid}`);
