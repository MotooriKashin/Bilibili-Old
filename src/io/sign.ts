 ...
 * 
 * @param url 未签名的url
 * @param appkey 签名所属平台的appkey，参看{@link APP_SECRET}
 * @returns 已签名的url
 */
export function sign<T extends URL | URLSearchParams>(
    url: T,
    appkey: keyof typeof APP_SECRET
): T {
    if (url instanceof URLSearchParams) {
        url.set('appkey', appkey);
        url.delete('sign');
        url.sort();
        url.set('sign', md5(url.toString() + APP_SECRET[appkey]));
        return url;
    }
    url.searchParams.set('appkey', appkey);
    url.searchParams.delete('sign');
    url.searchParams.sort();
    url.searchParams.set('sign', md5(url.searchParams.toString() + APP_SECRET[appkey]));
    return url;
}
