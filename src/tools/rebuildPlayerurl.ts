        ...
        async appPlayurl(app: any) {
            ...
            (<any>this.playurl.dash.video).forEach((d: any, i: any, e: any) => {
                arr.push((async (d) => {
                    ...
                    if (d.SegmentBase) OBJ["sidx" + String(API.cid)][id] = [d.SegmentBase.Initialization, d.SegmentBase.indexRange]
                    if (!OBJ["sidx" + String(API.cid)][id]) {
                        ...
                        API.debug("DASH-video：", id, OBJ["sidx" + String(API.cid)][id]);
                    }
                    d.segment_base = {
            ...
            (<any>this.playurl.dash.audio).forEach((d: any, i: any, e: any) => {
                arr.push((async (d) => {
                    ...
                    if (!OBJ["sidx" + String(API.cid)][id]) {
                        ...
                        let indexRagneEnd = hex_data.indexOf('6d6f6f66') / 2 - 5;
                        OBJ["sidx" + String(API.cid)][id] = ['0-' + String(indexRangeStart - 1), String(indexRangeStart) + '-' + String(indexRagneEnd)];
                        API.debug("DASH-audio：", id, OBJ["sidx" + String(API.cid)][id]);
                    }
                    d.segment_base = {
                        initialization: OBJ["sidx" + String(API.cid)][id][0],
                        index_range: OBJ["sidx" + String(API.cid)][id][1]
