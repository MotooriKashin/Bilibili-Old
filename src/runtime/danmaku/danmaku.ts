...
class Danmaku {
    ...
    toXml(danmaku: (danmaku | danmakuNew)[]) {
        ...
        let xml = DM.reduce((s, d) => {
            ...
            return s;
        }, '<?xml version="1.0" encoding="UTF-8"?><i><chatserver>chat.api.bilibili.com</chatserver><chatid>' + (<any>window).cid + '</chatid><mission>0</mission><maxlimit>99999</maxlimit><state>0</state><real_name>0</real_name><source>e-r</source>\r\n');
        xml += "</i>";
        /**
    ...
    async getSegDanmaku(aid = (<any>window).aid, cid = (<any>window).cid) {
        try {
            ...
            if (aid && (aid != (<any>window).aid)) loadProgress.total = dmMeta.dmSge.total;
     ...
     * @param cid cid
     */
    async dmWebView(aid = (<any>window).aid, cid = (<any>window).cid) {
        try {
     ...
     * @param cid cid
     */
    async dmWebSeg(i: number, aid = (<any>window).aid, cid = (<any>window).cid) {
        try {
            const data = await xhr({
     ...
     * @param config 弹幕元数据
     */
    async specialDms(aid = (<any>window).aid, cid = (<any>window).cid, config?: any) {
     ...
     * @returns 解析好的弹幕数组
     */
    async getHistoryDanmaku(date: string, cid = (<any>window).cid) {
        if (!date || !uid) return;
        let dm = await xhr({
    ...
    saveDanmaku(dm: danmaku[], fileName?: string) {
        ...
        saveAs(data, `${fileName || storage.ss.getItem("title") || (<any>window).cid}${setting.danmakuSaveType === "xml" ? ".xml" : ".json"}`);
    }
}
/** 旧版播放器对应的弹幕对象 */
