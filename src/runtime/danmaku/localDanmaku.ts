import { readAs } from "../../include/file";
import { videoFloat } from "../player/videoFloat";
...
export class LocalMedia {
    ...
    async danmaku() {
        ...
        this.data.json.forEach(async (d, i) => {
            ...
            (<any>window).player?.setDanmaku(data, <any>this.data.xml[0] || Boolean(i) || setting.danmakuContact);
        })
        this.offset = 0; // 记录或重置弹幕偏移时间
        if (!(<any>window).player?.offsetDanmaku) return toast.error("绑定键盘事件失败：弹幕偏移组件丢失！")
        else {
            ...
            if (!this.keyboard) {
                ...
                document.addEventListener("keydown", (ev) => {
                    switch (ev.key) {
                        case ",":
                            (<any>window).player.offsetDanmaku(-1);
                            this.offset--;
