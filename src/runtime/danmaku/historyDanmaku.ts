...
export function historyDanmaku() {
    const id = xhrhookAsync("history?type=", (args) => { // 修复历史弹幕
        ...
        xhr({
            url: `https://api.bilibili.com/x/v2/dm/web/history/seg.so?type=1&oid=${(<any>window).cid}&date=${param.date}`,
            ...
            setting.downloadOther && pushDownload({
                ...
                up: "历史",
                down: `N/A`,
                callback: () => danmaku.saveDanmaku(dm, `${storage.ss.getItem("title") || (<any>window).cid}`)
            });
        }).catch((e: Error) => {
