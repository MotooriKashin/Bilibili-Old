...
import { xhr } from "../xhr";
import { bstarPlayurl } from "./bstar_playurl";
...
export function videoLimit() {
    xhrhookAsync("/playurl?", () => <any>API.limit || API.th, async (args, type) => { // 代理限制视频的请求
        let response: any; // 初始化返回值
        const obj = urlObj(args[1]); // 提取请求参数
        obj.seasonId && (API.ssid = <number>obj.seasonId);
        obj.episodeId && (API.epid = <number>obj.episodeId);
        obj.ep_id && (API.epid = <number>obj.ep_id);
        obj.aid && (API.aid = Number(obj.aid)) && (API.aid = <number>obj.aid);
        obj.avid && (API.aid = Number(obj.avid)) && (API.aid = <number>obj.avid);
        obj.cid && (API.cid = Number(obj.cid)) && (API.cid = <number>obj.cid);
