...
class ClosedCaption {
    ...
    initUI() {
        ...
        this.fontposition();
        this.fontopacrity();
        addCssEs("runtime/player/closedCaption.css");
        this.changeResize();
        this.changePosition();
    }
    /** 切换字幕样式 */
    ...
    /** 切换字幕大小 */
    changeResize() {
        this.resizeRate = this.setting.scale ? API.player.getWidth() / 1280 * 100 : 100;
        this.changeStyle();
    }
    /** 切换字幕位置 */
    changePosition() {
    ...
    language() {
        ...
        this.element.language.children[2].onclick = () => {
            this.caption.subtitle_url && fetch(this.caption.subtitle_url).then(d => {
                d.blob().then(d => {
                    saveAs(d, `${sessionStorage.getItem("title")}-${this.caption.lan_doc}.json`);
                })
            })
        }
    ...
    async getCaption(data: any) {
        try {
            ...
            if (this.caption) {
                // 只在有字幕时添加面板
                API.player.addEventListener('video_resize', (event: any) => {
    ...
    async setCaption(caption?: any) {
        ...
        API.player.updateSubtitle(data); // 投喂字幕数据给播放器
        setTimeout(() => {
            if (API.player.getState() == "PLAYING") {
                // 刷新一次播放状态
                API.player.pause();
                API.player.play();
            }
