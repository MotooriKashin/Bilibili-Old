...
new Toast();
const toast: Toast & Toast["info"] = <any>new Proxy((...msg: string[]) => { debug.debug(...msg); Toast.show("info", ...msg) }, {
    get: (t, p: "info" | "success" | "warning" | "error") => {
        let result: any = () => { };
        switch (p) {
            case "info": result = (...msg: string[]) => { debug.debug(...msg); Toast.show("info", ...msg) };
                break;
            case "success": result = (...msg: string[]) => { debug.log(...msg); Toast.show("success", ...msg) };
                break;
            case "warning": result = (...msg: string[]) => { debug.warn(...msg); Toast.show("warning", ...msg) };
                break;
            case "error": result = (...msg: string[]) => { debug.error(...msg); Toast.show("error", ...msg) };
                break;
        }
        return result;
    }
