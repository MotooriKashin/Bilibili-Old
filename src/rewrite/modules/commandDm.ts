 ...
 */
(function () {
    API.addCss(GM.getResourceText("commandDm.css"));
     ...
     */
    function init() {
        if ((<any>window).__INITIAL_STATE__ && __INITIAL_STATE__.videoData && (<any>window).player) {
    ...
    class biliAPI {
        ...
        static triple() {
            return post("//api.bilibili.com/x/web-interface/archive/like/triple", {
                ...
                if (!d.coin) API.toast.error("投币失败");
                if (!d.like) API.toast.error("点赞失败");
                if (!d.fav) API.toast.error("收藏失败");
                return d;
            });
        }
    }
