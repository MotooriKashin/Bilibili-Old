...
(function () {
    if (API.path[5].startsWith("ml")) {
        const ml = Number((<any[]>API.path[5].match(/[0-9]+/))[0]);
        // 保存收藏号并调用av跳转
        if (!config.medialist) return;
        GM.setValue("medialist", ml);
        return API.runWhile(() => (<any>window).aid, () => location.replace(`https://www.bilibili.com/video/${(<any>window).aid}`))
    }
    // 新版稍后再看跳转到旧版稍后再看
    if (API.path[5].startsWith("watchlater") && config.watchlater) location.replace("https://www.bilibili.com/watchlater/#/");
    ...
    try {
        ...
        API.xhr({ url: `https://api.bilibili.com/x/v1/medialist/resource/ids4Player?media_id=${mid}` }).then(async (d: any) => {
            ...
            while (avs.length) {
                ...
                promises.push(API.xhr({ url: API.objUrl("https://api.bilibili.com/x/article/cards", { "ids": value.join("%2C") }) }));
            }
            value = [];
            ...
            API.runWhile(() => (<any>window).BilibiliPlayer, () => {
                ...
                API.debug("收藏列表", toview);
                API.toast.success("重构成功！刷新播放器...");
    ...
    } catch (e) { API.debug.trace(e, "mediaList.js"), true }
})();