...
(function () {
    ...
    BLOD.reset = {
         ...
         */
        switchVideo: async () => {
            if (BLOD.path.name) BLOD.updateSubtitle();
    ...
    class ClosedCaption {
        constructor() {
            ...
            this.setting = BLOD.getValue("subtitle") || {
            ...
            this.subtitlePrefer = BLOD.getValue("subtitlePrefer");
        }
        /**
         * 绘制字幕面板
         */
        ...
        changeStyle() {
            ...
            span.subtitle-item {${this.configs.shadow[this.setting.shadow].style}}`, "caption-style");
            BLOD.setValue("subtitle", this.setting);
        }
        /**
         * 切换字幕大小
         */
        ...
        iconSwitch(caption) {
            if (caption) {
                ...
                this.element.language.children[2].removeAttribute("disable", "disable");
            }
            else {
                this.element.span.innerHTML = this.iconOff;
                this.setCaption();
                this.text.innerHTML = "关闭";
                this.element.language.children[2].setAttribute("disable", "disable");
            }
        }
        /**
         * 字幕选择
        ...
        language() {
            ...
            list.children[0].onclick = () => this.setCaption();
            for (let i = this.captions.length - 1; i >= 0; i--) {
                let temp = BLOD.addElement("div", { class: "bpui-selectmenu-txt", "data-value": this.captions[i].lan }, list, true);
                temp.onclick = () => {
                    this.iconSwitch(this.captions[i]);
                    BLOD.setValue("subtitlePrefer", this.subtitlePrefer = this.captions[i].lan);
                }
            }
        }
        /**
        ...
        fontcolor() {
            ...
            this.element.fontcolor.children[1].children[0].innerHTML = this.color[this.setting.color];
        ...
        fontshadow() {
            ...
            this.element.fontshadow.children[1].children[0].innerHTML = this.shadow[this.setting.shadow];
        ...
        fontposition() {
            ...
            this.element.fontposition.children[1].children[0].innerHTML = this.position[this.setting.position];
        ...
        async getCaption() {
            try {
                ...
                let data = BLOD.jsonCheck(await BLOD.xhr(BLOD.objUrl("https://api.bilibili.com/x/player/v2", { cid: BLOD.cid, aid: BLOD.aid })));
                this.captions = data.data.subtitle.subtitles;
                let i = 0;
                this.captions.forEach((d, j) => {
                    if (d.lan == this.subtitlePrefer) i = j;
                })
                await this.setCaption(this.captions[0])
                if (this.captions) {
        ...
        async setCaption(caption) {
            let data = (caption && caption.subtitle_url) ? await BLOD.xhr(caption.subtitle_url, "json") : {}
            this.player.updateSubtitle(data);
    ...
    BLOD.updateSubtitle = () => { return new ClosedCaption().getCaption() }
})()
