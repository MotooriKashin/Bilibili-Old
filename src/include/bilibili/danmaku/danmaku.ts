interface modules {
    /** 弹幕反射表 */
    readonly "bilibiliDanmaku.json": Record<PropertyKey, any>
}
namespace API {
    // 启动protobuf引擎
    if (!GM.protobuf) {
    ...
    const danmakuType = new Proxy(<Record<nested, {
        ...
        get: (t, p: nested, r) => {
            if (!t[p]) {
                if (!root) {
                    debug.error("protobuf 框架异常~", "无法解析protobuf弹幕 ಥ_ಥ");
                    return;
                }
    ...
    class Danmaku {
        ...
        async getSegDanmaku(aid = API.aid, cid = API.cid) {
            try {
                ...
                config.commandDm && dmMeta.commandDms && Promise.resolve().then(() => { loadCommandDm(dmMeta.commandDms, aid, cid) })
                return result;
            } catch (e) {
                toast.error("加载弹幕出错~");
        ...
        saveDanmaku(dm: danmaku[], fileName?: string) {
            let data = config.danmakuSaveType === "xml" ? this.toXml(dm) : JSON.stringify(dm, undefined, '\t');
            saveAs(data, `${fileName || title || cid}${config.danmakuSaveType === "xml" ? ".xml" : ".json"}`);
        }
    }
    /** 旧版播放器对应的弹幕对象 */
