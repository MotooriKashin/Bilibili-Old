    ...
    class AllDanmaku {
        ...
        async init() {
            try {
                ...
                danmaku.reverse();
                // 取最早一条弹幕的时间
                this.time = timeFormat(danmaku[danmaku.length - 1].ctime * 1000, true).split(" ")[0];
            ...
            try {
                ...
                if (data && data[0]) {
                    ...
                    if (this.timeT) {
                        ...
                        this.noteChange("info", [`技能冷却中。。。请稍待 ${config.allDanmaku.delay} 秒钟`]);
                        return setTimeout(() => this.init(), config.allDanmaku.delay * 1000);
                    } else {
                        ...
                        this.noteChange("info", [`获取前一个月数据 ${this.arrT.slice(0, 2).join("-")} 请稍待 ${config.allDanmaku.delay} 秒钟`]);
                        return setTimeout(() => this.check(), config.allDanmaku.delay * 1000);
                    }
                } else {
        ...
        async done(boolean?: any) {
            ...
            config.downloadOther && pushDownload({
                ...
                up: "全弹幕",
                down: `N/A`,
                callback: () => API.danmaku.saveDanmaku(danmaku, `[全弹幕]${title || cid}`)
            });
        }
    }
    /** 全弹幕装填 */
