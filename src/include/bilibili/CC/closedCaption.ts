...
namespace API {
    class ClosedCaption {
        ...
        initUI() {
            ...
            this.fontposition();
            this.fontopacrity();
            addCss(getModule("closedCaption.css"), "caption");
            this.changeResize();
            this.changePosition();
        }
        /** 切换字幕样式 */
        ...
        language() {
            ...
            <a class="bpui-button" href="https://member.bilibili.com/v2#/zimu/my-zimu/zimu-editor?cid=${cid}&aid=${aid}" target="_blank" title="" style="margin-right: 0px; height: 24px; padding: 0px 6px;">添加字幕</a>`;
            ...
            this.element.language.children[2].onclick = () => {
                this.caption.subtitle_url && fetch(this.caption.subtitle_url).then(d => {
                    d.blob().then(d => {
                        saveAs(d, `${title}-${this.caption.lan_doc}.json`);
                    })
                })
            }
        ...
        fontcolor() {
            ...
            this.color.forEach(d => {
                ...
                let temp = addElement("li", { class: "bpui-selectmenu-list-row", "data-value": d.value }, this.element.fontcolor.children[1].children[2]);
        ...
        fontshadow() {
            ...
            this.shadow.forEach(d => {
                ...
                let temp = addElement("li", { class: "bpui-selectmenu-list-row", "data-value": d.value }, this.element.fontshadow.children[1].children[2]);
                temp.innerHTML = d.content;
                temp.onclick = () => {
        ...
        fontposition() {
            ...
            this.position.forEach(d => {
                ...
                let temp = addElement("li", { class: "bpui-selectmenu-list-row", "data-value": d.value }, this.element.fontposition.children[1].children[2]);
                temp.innerHTML = d.content;
                temp.onclick = () => {
        ...
        convertion(arr: any[]) {
            ...
            arr.forEach(d => {
                ...
                config.downloadOther && pushDownload({
                    ...
                    up: d.lan,
                    down: d.lan_doc,
                    fileName: `${title || `av${aid}`}-${d.lan_doc}.json`
                });
            });
            if (chs || !base) return;
            base.lan = "zh-CN";
    ...
    /** 为旧版播放器追加CC字幕功能 */
    export const closedCaption = new ClosedCaption();
}