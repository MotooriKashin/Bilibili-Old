interface modules {
    /** 全弹幕装填 */
    readonly "allDanmaku.js": string;
}
namespace API {
    export class AllDanmaku {
        pubdate: any; // 视频发布时间
        today: any; // 当天
        time: any; // 备份
        arrP: any; // 备份
        ...
        constructor(callback?: Function) {
            ...
            if ((<any>API).__INITIAL_STATE__) {
                if ((<any>API).__INITIAL_STATE__.videoData && (<any>API).__INITIAL_STATE__.videoData.pubdate) {
                    this.pubdate = new Date(1E3 * (<any>API).__INITIAL_STATE__.videoData.pubdate);
                } else if ((<any>API).__INITIAL_STATE__.epInfo && (<any>API).__INITIAL_STATE__.epInfo.pub_real_time) {
                    this.pubdate = new Date((<any>API).__INITIAL_STATE__.epInfo.pub_real_time);
                }
            } else {
                let time = document.querySelector("div.tm-info.tminfo > time");
                time && (this.pubdate = new Date(time.innerHTML));
            }
            this.pubdate = Format.timeFormat(<any>this.pubdate, true).split(" ")[0]; // 视频上传日期
         ...
         */
        async init() {
            if (!API.uid) return toast.warning("本功能需要登录！")
            ...
            try {
                ...
                let danmaku = <danmakuNew[]>await API.danmaku.getHistoryDanmaku(this.time);
