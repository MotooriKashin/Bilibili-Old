...
namespace API {
    ...
    class Aria2 {
        ...
        constructor() {
            ...
            config.aria2.token && (this.setting.token = config.aria2.token);
        }
        /**
         * 生成aria2命令行参数并赋值到剪切板
         * @param obj 下载配置数据
        ...
        postMessage<T extends keyof Aria2Method>(method: T, id: string, params: any[] = []): Promise<ReturnType<Aria2Method[T]>> {
            const url = `${config.aria2.server}:${config.aria2.port}/jsonrpc`;
            config.aria2.token && params.unshift(`token:${config.aria2.token}`);
