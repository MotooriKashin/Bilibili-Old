...
(function () {
    class AllDanmaku {
        ...
        async done(boolean?: any) {
            ...
            if (boolean) toast.success("全弹幕获取成功，正在装填。。。", "总弹幕量：" + API.unitFormat(this.danmaku.length), "同时推送至下载面板，可右键保存 π_π");
            window.player?.setDanmaku(danmaku);
            API.danmaku = danmaku;
            this.callback && this.callback();
        }
    }
    API.allDanmaku = (callback?: Function) => {
        try {
            new AllDanmaku(callback);
        } catch (e) { toast.error("allDanmaku.js", e) }
    };
})();
