...
(function () {
    ...
    class DanmakuHashId {
        ...
        async getInfo() {
            try {
                ...
                DanmakuHashId.catch[this.mid] = DanmakuHashId.catch[this.mid] || API.jsonCheck(await xhr({ url: API.objUrl("https://api.bilibili.com/x/web-interface/card", { mid: <any>this.mid }) }));
                ...
                DanmakuHashId.count--;
            } catch (e) { DanmakuHashId.count--; toast.error("danmakuHashId.js", e) }
        }
    }
    window.danmakuHashId = (crc: string) => {
        ...
        } catch (e) { toast.error("danmakuHashId.js", e) }
    }
})();
interface Window {
    /**
