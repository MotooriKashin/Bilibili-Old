...
namespace API {
    class Read extends Rewrite {
        ...
        head() {
            ...
            <a class="category-link" href="//www.bilibili.com/read/${(<any>this.bar.find(d => {
                if (d[0] == this.data.readInfo.category.parent_id) return d;
        ...
        origin() {
            ...
            (<HTMLDivElement>document.querySelector(".page-container")).innerHTML = this.temp; // 修补html模板
            this.flushDocument();
            runWhile(() => document.body, () => importModule("user-select.js")); // 解锁复制/右键
        }
    }
    new Read("read.html");
}