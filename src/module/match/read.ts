...
(function () {
    ...
    try { } catch (e) { API.trace(e, "read.js") }
    API.path.name = "read";
    (<Promise<string>>xhr({ url: location.href, async: false })).then(d => {
        let data = d.includes("__INITIAL_STATE__=") ? JSON.parse(d.match(/INITIAL_STATE__=.+?\;\(function/)[0].replace(/INITIAL_STATE__=/, "").replace(/;\(function/, "")) : "";
        ...
        let body = `<div class="article-holder">${data.readInfo.content}</div><p class="original">本文为我原创</p>`;
        let tag = (data.readInfo.tags || []).reduce((o: string, d: any) => {
            // 构造tag
        ...
        API.rewriteHTML(API.getModule("read.html").replace(`<div class="page-container"></div>`, data));
    });
})();