...
import { aria2 } from "../runtime/download/aria2.js";
import { AccessKey } from "./accessKey";
import { UPOS } from "../runtime/player/uposReplace";
...
/** 设置数据配置 请将默认数导出一份到./include/setting.json供背景脚本使用 */
            ...
            },
            {
                ...
                change: (v: string) => {
                    chrome.storage.local.get().then(d => {
                        d.toast && toast[<"info">d.toast.type](v);
                    })
                }
            }
        ]
        ...
        list: [
            ...
            },
            {
                ...
                callback: (v: any) => {
                    if (v) {
                        chrome.storage.local.get().then(d => {
                            d.automate.webFullScreen = false
