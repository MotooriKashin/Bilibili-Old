...
export class Aria2 {
    ...
    cmdlet(data: IAria2Data) {
        const arr = ['aria2c'];
        data.urls.forEach(d => arr.push(`"${d}"`));
        data.out && arr.push(`--out="${data.out}"`);
        ...
        (data.dir || this.dir) && arr.push(`--dir="${data.dir || this.dir}"`);
        (data.split || this.split) && arr.push(`--split="${data.split || this.split}"`, `--max-connection-per-server="${data.split || this.split}"`);
        (data.size || this.size) && arr.push(`--min-split-size="${data.size || this.size}M"`);
        data.header && Object.entries(data.header).forEach(d => arr.push(`--header="${d[0]}: ${d[1]}"`));
    ...
    private postMessage<T extends keyof Aria2Method>(method: T, id = getMetux(), params: any[] = []): Promise<ReturnType<Aria2Method[T]>> {
