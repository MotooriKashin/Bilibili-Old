...
(function () {
    ...
    class Ui {
        ...
        static action(obj: ItemPus, node?: Element) {
            ...
            obj.float && this.float(div, obj.float);
            node && node.appendChild(div);
            (div.querySelector(".xaction") as HTMLDivElement).onclick = () => {
                (<HTMLDivElement>div.querySelector(".xaction")).setAttribute("disabled", "disabled");
                disabled && setTimeout(() => (<HTMLDivElement>div.querySelector(".xaction")).removeAttribute("disabled"), disabled * 1000);
                obj.action.call(div);
            }
            return div;
        }
        ...
        static input(obj: ItemIpt, node?: Element) {
            ...
            obj.title && ((div.querySelector(".xbutton") as HTMLDivElement).onclick = () => {
                if (!input.value || ((<any>config)[obj.key] == input.value)) return;
                if (obj.pattern && !obj.pattern.test(input.value)) return API.toast.warning("非法输入！", `正则限制：${obj.pattern.toString()}`);
                (<HTMLDivElement>div.querySelector(".xbutton")).setAttribute("disabled", "disabled");
                disabled && setTimeout(() => (<HTMLDivElement>div.querySelector(".xbutton")).removeAttribute("disabled"), disabled * 1000);
                obj.hasOwnProperty("value") && (obj.value = input.value, (<any>config)[obj.key] = input.value);
                !history.includes(input.value) && history.push(input.value) && (this.history[obj.key] = history);
                !obj.action && API.toast.warning(`数值已变更：${input.value}`);
                obj.action && obj.action.call(div, input.value);
        ...
        static file(obj: ItemFie, node?: Element) {
            ...
            obj.float && this.float(div, obj.float);
            node && node.appendChild(div);
            (div.querySelector(".xbutton") as HTMLDivElement).onclick = () => input.click();
