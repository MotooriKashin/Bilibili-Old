...
(function () {
    ...
    API.bubbleSort = (arr: number[]) => Format.bubbleSort(arr);
    API.randomArray = (arr: any[], num: number) => Format.randomArray(arr, num);
    API.objUrl = (url: string, obj: { [name: string]: string }) => Format.objUrl(url, obj);
    API.urlObj = (url?: string) => Format.urlObj(url);
})();
declare namespace API {
     ...
     * @returns 时:分:秒 | 年-月-日 时:分:秒
     */
    function timeFormat(time?: number, type?: boolean | undefined): string;
    /**
     * 格式化字节
     * @param size 字节/B
     * @returns n B | K | M | G
