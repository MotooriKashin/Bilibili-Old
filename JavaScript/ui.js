...
(function () {
    ...
    class Ui {
        constructor() {
            ...
            this.item = [
                ["widescreen", "electric", "panel", "autoplay", "playerStyle", "forceFlv", "closedCaption"],
        ...
        async accesskey() {
            ...
            if (!config.reset.accesskey) {//
                if (BLOD.GM.getValue("access_key")) {
                    BLOD.GM.deleteValue("access_key");
                    BLOD.GM.deleteValue("access_date");
            ...
            if (!BLOD.GM.getValue("access_key") || (Date.now() - BLOD.GM.getValue("access_date") > 2160000)) {
                try {
                    ...
                    document.body.appendChild(page);
                    setTimeout(() => { page.remove() }, 3000);
                    BLOD.GM.setValue("access_key", data.access_key);
                    BLOD.GM.setValue("access_date", Date.now());
        ...
        danmaku() {
            if (window.player && BLOD.path.name) {
                ...
                let delay = BLOD.GM.getValue("allDanmaku") || {};
                ...
                step.oninput = () => {
                    ...
                    BLOD.GM.setValue("allDanmaku", delay);
                }
            }
        }
        /**
        ...
        limit() {
            ...
            li.onclick = () => {
                ...
                input.value = decodeURI(BLOD.GM.getValue("thaiLand") || "");
                let callback = () => {
                    if (input.value && /^http.+$/.test(input.value)) {
                        ...
                        BLOD.GM.setValue("thaiLand", encodeURI(input.value));
