...
(function () {
        ...
        this.addEventListener('readystatechange', () => {
            if (this.readyState === 4) {
                try {
                    ...
                    API.debug.trace(e, "restoreData.js");
                }
            }
        });
    ...
    async function restoreBangumiRecommand() {
        let data = API.jsonCheck(await API.xhr({ url: API.objUrl("https://api.bilibili.com/pgc/web/recommend/related/recommend", { season_id: String(API.__INITIAL_STATE__.ssId) }) })).result;
        let result = API.jsonCheck(await API.xhr({ url: API.objUrl("https://api.bilibili.com/x/tag/info", { tag_name: API.__INITIAL_STATE__.mediaInfo.title }) }));
