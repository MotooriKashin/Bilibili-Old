...
// @resource     bilibiliPlayer.js https://cdn.jsdelivr.net/gh/MotooriKashin/Bilibili-Old@431988b536837afc4dae6ca09fe7c3f87ef5c392/dist/bilibiliPlayer.min.js
...
const config = new Proxy({}, {
    set: (_target, p, value) => {
        _target[p] = value;
        GM_setValue("config", _target);
        return true;
    }
});
Object.entries(GM_getValue("config", {})).forEach(k => config[k[0]] = k[1]);
/**
...
const debug = new Proxy(Debug, { apply: (target, thisArg, argArray) => { target.log(...argArray); } });
/**
 * URL处理函数
 */
class Url {
...
Toast.sence = 60;
new Toast();
const toast = new Proxy((...msg) => { debug.debug(...msg); Toast.show("info", ...msg); }, {
    get: (t, p) => {
        let result = () => { };
        switch (p) {
            case "info":
                result = (...msg) => { debug.debug(...msg); Toast.show("info", ...msg); };
                break;
            case "success":
                result = (...msg) => { debug.log(...msg); Toast.show("success", ...msg); };
                break;
            case "warning":
                result = (...msg) => { debug.warn(...msg); Toast.show("warning", ...msg); };
                break;
            case "error":
                result = (...msg) => { debug.error(...msg); Toast.show("error", ...msg); };
                break;
        }
        return result;
    }
...
Xhr.catches = [];
Xhr.log = () => _a.catches;
const xhr = new Proxy(Xhr, { apply: (target, thisArg, argArray) => { return target.xhr.apply(target, argArray); } });
...
`(function () {
    ...
    class Window {
        ...
        importModule(name, args = {}) {
            if (!name)
                return Object.keys(modules).filter(p => typeof modules[p] === "string"); // 空调用返回所有可用模块
            if (typeof modules[name] === "string") { // 有效调用
                new Function("API", "GM", "config", "debug", "Format", "toast", "xhr", ...Object.keys(args), modules[name])(API, GM, config, debug, Format, toast, xhr, ...Object.keys(args).reduce((s, d) => {
                    s.push(args[d]);
                    return s;
                }, []));
            }
            else
                toast.warning(\`模块\${name}并不存在！是不是拼写错了？\`); // 无效调用
        }
        registerSetting(obj) {
            SETTING.push(obj);
        ...
        biliQuickLogin() {
            window.biliQuickLogin ? window.biliQuickLogin() : this.loadScript("//static.hdslb.com/account/bili_quick_login.js", () => this.biliQuickLogin());
        }
        ...
        async saveAs(content, fileName, contentType = "text/plain") {
        ...
        readAs(file, type = "string", encoding) {
            return new Promise((resolve, reject) => {
        ...
        async alertMessage(text, title = this.Name) {
...
`try {
    ...
    if (!disable) {
        ...
        if (config.medialist && /\\/medialist\\/play\\//.test(location.href) || /\\/playlist\\/video\\/pl/.test(location.href))
        ...
        async getInfo() {
            try {
                ...
                if (this.dm.tagName != "LI")
                    return;
                DanmakuHashId.catch[this.mid] = DanmakuHashId.catch[this.mid] || API.jsonCheck(await xhr({ url: Format.objUrl("https://api.bilibili.com/x/web-interface/card", { mid: this.mid }) }));
...
`class LocalMedia {
    ...
    async danmaku() {
        ...
        this.data.xml.forEach(async (d, i) => {
            ...
            API.danmaku.loadLocalDm(data, Boolean(i) || config.concatDanmaku);
        });
...
`class ParameterTrim {
    ...
    triming(url) {
        const obj = Format.urlObj(url);
        obj.bvid && (obj.aid = API.abv(obj.bvid), obj.bvid = undefined);
        obj.aid && !Number(obj.aid) && (obj.aid = API.abv(obj.aid));
...
`API.switchVideo(() => {
    ...
    xhr({
        ...
        return xhr({
            url: Format.objUrl("https://api.bilibili.com/x/v2/dm/view", { oid: API.cid, aid: API.aid, type: 1 }),
            responseType: "json",
            credentials: true
        });
    }).then((data) => {
        API.runWhile(() => ready, () => {
            ...
            // 分段进度条
            config.segProgress && ((_c = data === null || data === void 0 ? void 0 : data.data) === null || _c === void 0 ? void 0 : _c.view_points[1]) && new API.segProgress(data);
        });
    });
});
...
`{
    ...
    API.xhrhookasync("/playurl?", args => {
        ...
        if (API.globalLimit) {
            ...
            obj.module = (((_b = (_a = API.__INITIAL_STATE__) === null || _a === void 0 ? void 0 : _a.upInfo) === null || _b === void 0 ? void 0 : _b.mid) == 1988098633 || ((_d = (_c = API.__INITIAL_STATE__) === null || _c === void 0 ? void 0 : _c.upInfo) === null || _d === void 0 ? void 0 : _d.mid) == 2042149112) ? "movie" : "bangumi";
        ...
        else if (API.pgc && ((_f = (_e = API.__INITIAL_STATE__) === null || _e === void 0 ? void 0 : _e.rightsInfo) === null || _f === void 0 ? void 0 : _f.watch_platform)) {
            obj.access_key = accesskey;
            obj.fnval = null;
            obj.fnver = null;
...
`{
    class NodeHook {
        ...
        jsonphook(url, redirect, modifyResponse, once = true) {
            let id;
        ...
        jsonphookasync(url, condition, modifyResponse, once = true) {
            let id;
            const one = Array.isArray(url) ? url : [url];
            const two = function () {
                try {
        ...
        scriptIntercept(url, redirect, text) {
            const one = Array.isArray(url) ? url : [url];
            const two = function () {
                try {
                    if (text) {
    ...
    API.jsonphook = (url, redirect, modifyResponse, once) => nodehook.jsonphook(url, redirect, modifyResponse, once);
    API.jsonphookasync = (url, condition, modifyResponse, once) => nodehook.jsonphookasync(url, condition, modifyResponse, once);
    API.scriptBlock = (url) => nodehook.scriptBlock(url);
    API.scriptIntercept = (url, redirect, text) => nodehook.scriptIntercept(url, redirect, text);
...
`class AllDanmaku {
    constructor(callback) {
        ...
        this.pubdate = Format.timeFormat(this.pubdate, true).split(" ")[0]; // 视频上传日期
        this.today = Format.timeFormat(undefined, true).split(" ")[0]; // 当天日期
    ...
    async init() {
        ...
        try {
            ...
            danmaku.reverse();
            // 取最早一条弹幕的时间
            this.time = Format.timeFormat(danmaku[danmaku.length - 1].ctime * 1000, true).split(" ")[0];
            this.danmaku = this.danmaku.concat(danmaku);
            toast("数据返回！已获取弹幕数：" + Format.unitFormat(this.danmaku.length));
            this.arrT = this.time.split("-");
            // 如果当天不是投稿日，转入日期检查
    ...
    async check() {
        try {
            ...
            // 日期未早于投稿日，正常请求月份数据
            let data = await xhr({
                url: Format.objUrl("https://api.bilibili.com/x/v2/dm/history/index", {
                    type: 1,
                    oid: API.cid,
    ...
    async done(boolean) {
        ...
        if (boolean)
            toast.success("全弹幕获取成功，正在装填。。。", "总弹幕量：" + Format.unitFormat(this.danmaku.length), "同时推送至下载面板，可右键保存 π_π");
...
`{
    ...
    API.mediaSession = (data) => mediaSession(data);
}
        ...
        this.playurl.dash.video.forEach((d, i, e) => {
            arr.push((async (d) => {
                ...
                if (!RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id]) {
                    ...
                    debug("DASH-video：", id, RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id]);
                }
                d.segment_base = {
        ...
        this.playurl.dash.audio.forEach((d, i, e) => {
            arr.push((async (d) => {
                ...
                if (!RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id]) {
                    ...
                    let indexRagneEnd = hex_data.indexOf('6d6f6f66') / 2 - 5;
                    RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id] = ['0-' + String(indexRangeStart - 1), String(indexRangeStart) + '-' + String(indexRagneEnd)];
                    debug("DASH-audio：", id, RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id]);
                }
                d.segment_base = {
                    initialization: RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id][0],
                    index_range: RebuildPlayerurl.OBJ["sidx" + String(API.cid)][id][1]
    ...
    API.registerSetting({
        ...
        action: () => {
            if (API.aid && API.cid) {
                ...
                toast.warning("当前并非视频页，请在视频页面打开！");
        }
    });
    API.registerSetting({
        type: "switch",
...
`{
    ...
    API.urlInputCheck = async function (input) {
        ...
        let obj = Format.urlObj(input); // 获取参数对象
        ...
        try {
            if (aid) {
                ...
                if (!cid) {
                    try {
                        // 尝试访问B站服务器获取信息
                        catchs.aid[aid] = API.jsonCheck(await xhr({ url: Format.objUrl("https://api.bilibili.com/x/player/pagelist", { "aid": aid }) })).data;
                        ...
                        try {
                            // 尝试访问BiliPlus获取信息
                            let data = API.jsonCheck(await xhr({ url: Format.objUrl("https://www.biliplus.com/api/view", { "id": aid }) }));
                ...
                try {
                    ...
                    let data;
                    if (epid)
                        data = JSON.stringify(catchs.epid[epid]) || await xhr({ url: Format.objUrl("https://bangumi.bilibili.com/view/web_api/season", { ep_id: epid }) });
                    else if (ssid)
                        data = JSON.stringify(catchs.ssid[ssid]) || await xhr({ url: Format.objUrl("https://bangumi.bilibili.com/view/web_api/season", { season_id: ssid }) });
                    if (data) {
...
API.abv = (input) => new Abv().check(input);
...
}
{
    ...
    API.midcrc = input => crc.run(input);
    API.crc32 = input => (((crc.crc32(input) + 1) * -1) >>> 0).toString(16);
}
...
`class Player extends API.rewrite {
    constructor(html) {
        super(html);
        this.obj = Format.urlObj(location.href);
    ...
    async afterFlush() {
        ...
        if (!Number(this.cid)) {
            ...
            this.cid = this.obj.cid;
            if (!Number(this.cid))
                return toast.error("初始化嵌入式播放器失败！", this.obj);
        }
        const playerParam = {
            aid: this.aid,
            cid: this.cid,
...
`class Av extends API.rewrite {
    ...
    async getIniState() {
        ...
        if (this.__INITIAL_STATE__.videoData.redirect_url)
            return toast.warning("番剧重定向...", this.__INITIAL_STATE__.videoData.redirect_url);
...
`API.runWhile(() => document.querySelector(".v.play") && API.aid, async () => {
    try {
        ...
        // 初始化按钮
        let data = await xhr({
            url: Format.objUrl("https://api.bilibili.com/x/web-interface/view", { aid: API.aid }),
            credentials: true
        });
        ...
        data = API.jsonCheck(await xhr({
            url: Format.objUrl("https://api.bilibili.com/x/web-interface/archive/has/like", { "aid": API.aid }),
            credentials: true
        })).data;
        if (data == 1) {
            // 点赞过点亮图标
...
`{
    ...
    let progress = Number(Format.urlObj(location.href).dm_progress);
    let first = 0;
    API.switchVideo(async () => {
        var _a;
...
`class Bangumi extends API.rewrite {
    ...
    async getIniState() {
        ...
        config.bangumiEplist && ((_c = this.__INITIAL_STATE__) === null || _c === void 0 ? void 0 : _c.epList[1]) && (this.__INITIAL_STATE__.special = false, this.__INITIAL_STATE__.mediaInfo.bkg_cover = undefined);
        this.appendIniState();
...
`{
    ...
    async function restoreBangumiRecommand() {
        let data = API.jsonCheck(await xhr({ url: Format.objUrl("https://api.bilibili.com/pgc/web/recommend/related/recommend", { season_id: String(API.__INITIAL_STATE__.ssId) }) })).result;
        let result = API.jsonCheck(await xhr({ url: Format.objUrl("https://api.bilibili.com/x/tag/info", { tag_name: (API.__INITIAL_STATE__).mediaInfo.title }) }));
            ...
            prev.onclick = next.onclick = async () => {
                ...
                for (let i = indexFlag - 1; i >= indexFlag - 10; i--) {
                    ...
                    API.addElement("div", { class: "groom-module home-card" }, node, undefined, true).innerHTML = \`<a href="//www.bilibili.com/video/av\${indexRecommend[i].aid || indexRecommend[i].id}" target="_blank" title="\${indexRecommend[i].title}">
                        ...
                        "><!----><div class="card-mark"><p class="title">\${indexRecommend[i].title}</p><p class="author">up主：\${indexRecommend[i].owner.name}</p><p class="play">播放：\${Format.unitFormat(indexRecommend[i].stat.view)}</p></div></a><div class="watch-later-trigger w-later"></div></div>\`;
                }
...
`(async function () {
    try {
        ...
        let data = API.jsonCheck(await xhr.GM({ url: Format.objUrl("https://account.bilibili.com/api/member/getCardByMid", { "mid": API.mid }) }));
