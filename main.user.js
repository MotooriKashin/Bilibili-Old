    ...
    class ClosedCaption {
        ...
        async setCaption(caption) {
            ...
            if (caption && caption.subtitle_url) {
                ...
                API.bofqiMessage(["载入字幕", this.caption.lan_doc]);
            }
            else
...
`"use strict";
    ...
    const root = window.protobuf.Root.fromJSON(JSON.parse(protobufJSON));
    const danmakuElem = window.protobuf.Root.fromJSON(JSON.parse(danmakuJSON)).lookupType("bilibili.broadcast.message.main.DanmukuEvent");
    let sequence = 1;
    const message = {
...
/*!***********************!*/
/**/modules["cht2chs.js"] = /*** ./dist/include/cht2chs.js ***/
`"use strict";
    /** 繁简对照表，可能需要及时更新 */
    const aTC2SC = {
        "以功覆過": "以功复过",
...
//# sourceURL=API://@Bilibili-Old/include/cht2chs.js`;
/*!***********************!*/
                ...
                action: v => {
                    switch (v) {
                        case "默认":
                            ...
                            break;
                        case "警告":
                            API.toast.warning("警告通知", "黄~");
                            break;
                        case "错误":
                            API.toast.error("错误通知", "红~");
                            break;
                        case "成功":
                            API.toast.success("成功通知", "绿~");
                            break;
                    }
                }
            }
...
`"use strict";
    class Toast {
        ...
        static show(type, ...msg) {
            ...
            setTimeout(() => {
                ...
                this.come(item);
                setTimeout(() => this.quit(item), API.config.toasttimeout * 1000);
            }, this.count * API.config.toaststep);
            this.count++;
        }
     ...
     * @param msg 消息字符串
     */
    function toast(...msg) { API.config.toastconsole && API.debug.debug(...msg); _.info(...msg); }
    API.toast = toast;
    toast.info = function (...msg) { API.config.toastconsole && API.debug.debug(...msg); _.info(...msg); };
    toast.success = function (...msg) { API.config.toastconsole && API.debug.log(...msg); _.success(...msg); };
    toast.warning = function (...msg) { API.config.toastconsole && API.debug.warn(...msg); _.warning(...msg); };
    toast.error = function (...msg) { API.config.toastconsole && API.debug.error(...msg); _.error(...msg); };
...
`"use strict";
    class Av extends API.Rewrite {
        constructor(html) {
            ...
            this.getIniState();
            this.onload = () => { this.afterFlush(); };
            API.addCss("#app > div > .bili-header-m.report-wrap-module {display:none}"); // 使上古顶栏移除不可见
