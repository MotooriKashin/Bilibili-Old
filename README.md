...
反馈问题请一并提供浏览器版本及控制台(f12->Consle)报错截图！
...
4. 旧版播放器已失去官方维护，版本停留在`2019-10-31 07:38:36`，所以新版播放器互动视频、全景视频、高能进度条等功能是不支持的，互动视频脚本已主动忽略不会启用旧版，全景视频无法移动视角，只有CC字幕已支持。
...
11. 旧版弹幕默认接口已获取不到 90 分钟以后的弹幕数据，对于时长大于 90 分钟的电影等视频，会出现 90 分钟以后无弹幕的情况，只能在设置里启用“新版弹幕”解决。另外旧版实时弹幕也已失效，同样可在设置里启用“实时弹幕”加以修复。
12. 默认“按热度排序”获取到的评论页数虚标，切换一次“按时间排序”即可恢复正常。
...
- 2021-05-02
  - 修改排行榜页面自适应的错误
  - 支持显示“精选评论”
  - 恢复评论楼层功能直接写入`comment.js` 去除设置中相关选项
- 2021-05-01
  - 修复导航栏消息样式
- 2021-04-30
  - 改进修复评论时间轴跳转方式
...
- 2021-04-24
  - 弹幕xml文件添加换行
- 2021-04-23
  - 修复番剧页面推荐视频封面比例不对的问题 *火花✨*
- 2021-04-21
  - 禁用新版滚动评论区，恢复原先评论翻页功能 *可在设置里选择是否启用*
- 2021-04-20
  - 精简优化模块
- 2021-04-19
  - 旧版嵌入播放器失效，替换为远古播放器
- 2021-04-18
  - 修复番剧视频推荐 *由于接口不完全兼容，占位数据会一闪而过*
  - 处理消息页面样式
  - 提供Bangumi背景开关
- 2021-04-16
  - 支持弹幕互动投票
...
- 2021-04-04
  ...
  - 调整弹幕排序
- 2021-04-02
  - 失效视频支持呼出下载面板，以便下载弹幕文件
- 2021-04-01
  - 按正确的方式判定danmaku[0].class存在；弹幕排序性能优化
  - 右键下载菜单添加鼠标反馈
  - 自动滚动到播放器可以不在子页面生效
- 2021-03-24
  - 修复旧版浏览器番剧页面设置界面复选框样式
- 2021-03-23
  - 彻底弃用BigInt排序以处理旧版浏览器报错
- 2021-03-22
  - 重构视频拦截功能
  - BigInt Polyfill
- 2021-03-20
